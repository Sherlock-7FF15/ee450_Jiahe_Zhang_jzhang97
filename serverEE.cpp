...
int main() {
    ...
    cout<<"The serverEE is up and running using UDP on port "<<portNum<<"."<<endl;
    ...
    while (1) {
        ...
        if (code.find(",") == -1) {
            cout<<"The serverEE received a request from the Main Server about the course information of "<<code<<endl;
        } else {
            cout<<"The serverEE received a request from the Main Server about the "<<code.substr(6).c_str()<<" of "<<code.substr(0, 5).c_str()<<"."<<endl;
        }
...
int readAndCheck(char buff[], char stor[5][200], char snd[]) {
    ...
    if (file.is_open()) {
        string line;
        int i = 0;
        while (getline(file, line)) { // read from text line by line
            char *p;
            char tmp[1024]; //store read data
            string ori = buff; //ori: EE450,Credit if it is normal query. EE450 if it is multi query
            if (ori.find(",") == -1) { // if it is multiple query
                // cout<<"Multi search."<<endl;
